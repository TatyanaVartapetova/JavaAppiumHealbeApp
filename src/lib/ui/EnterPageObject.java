...
public class EnterPageObject extends MainPageObject {
    ...
    private static final String
            SIGN_IN_BUTTON_ENTER_FORM_SCREEN = "com.healbe.healbegobe.debug:id/sign_in",
            EMAIL_FIELD = "com.healbe.healbegobe.debug:id/email",
            PASSWORD_FIELD = "com.healbe.healbegobe.debug:id/password",
            SIGN_IN_BUTTON_SIGN_IN_SCREEN = "com.healbe.healbegobe.debug:id/button",
            ...
            EMAIL_VALUE_PROFILE_SCREEN = "com.healbe.healbegobe.debug:id/email_value";

    public void signIn(String login, String password) {
        waitForElementAndClick(By.id(EMAIL_FIELD), "Cannot find email field", 5);
        waitForElementAndSendKeys(By.id(EMAIL_FIELD), login, "Cannot find and type into email field", 5);
        waitForElementAndSendKeys(By.id(PASSWORD_FIELD), password, "Cannot find and type into password field", 5);
        waitForElementAndClick(By.id(SIGN_IN_BUTTON_SIGN_IN_SCREEN), "Cannot click on Sign In Button on Sign In", 5);
        waitForElementNotPresent(By.id(EMAIL_FIELD), "email field still present after click on Sign In button", 15);
    ...
    public void signInAndAssertSignInSuccessful(String login, String password) {
       ...
       // while (driver.findElements(MobileBy.AccessibilityId("Close")).size() == 1) { ...} - если в этот цикл обернуть, то будет работать независимо от того показан ли квикстарт
        ...
        swipeUpToFindElement(By.id(EMAIL_VALUE_PROFILE_SCREEN), "Cannot Find email value on Profile", 20);
        assertElementHasText(By.id(EMAIL_VALUE_PROFILE_SCREEN), login, "login in Profile not equal to " + login);
    ...
    public  void signInWrongPassword(){
        ...
        assertElementPresent(By.id(EMAIL_FIELD), "Email field not present after input wrong password");
    ...
    public  void signInEmailNotValid(){
       // waitForElementAndClick(By.id(SIGN_IN_BUTTON_ENTER_FORM_SCREEN), "Cannot click on Sign In Button on Enter Form", 5);
        waitForElementAndClick(By.id(EMAIL_FIELD), "Cannot find email field", 5);
        waitForElementAndSendKeys(By.id(EMAIL_FIELD), "v60@v.", "Cannot find and type into email field", 5);
        waitForElementAndClick(By.id(PASSWORD_FIELD), "Cannot click to password fields", 5);
        ...
        assertElementPresent(By.id(EMAIL_FIELD), "Email field not present after input wrong password");
        assertButtonIsNotEnable(By.id(SIGN_IN_BUTTON_SIGN_IN_SCREEN), "bla", 5);
    ...
    public void clearEmailAndPasswordFields(){
        waitForElementAndClear(By.id(EMAIL_FIELD), "Cannot clear email field", 5);
        waitForElementAndClear(By.id(PASSWORD_FIELD), "Cannot clear password field", 5);
    }
