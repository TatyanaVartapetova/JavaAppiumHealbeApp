...
public class OnboardingPageObject extends MainPageObject{
    ...
    SIGN_IN_BUTTON = "com.healbe.healbegobe.debug:id/sign_in",
  ...
  private static String getOnboardingPage2TextXpath(String description_onboarding2){
    ...
    public void navigationNext(){
        this.waitForElementAndClick(By.xpath(NAVIGATE_NEXT_BUTTON), "cannot find Navigate next button", 15);
    }

    public void passOnboardingQuick(){
      // this.doubleTapToCenterOfElement(By.xpath("//androidx.compose.ui.platform.ComposeView/android.view.View/android.view.View/android.view.View"), "Cannot find center of element gif  onboarding"); - почему-то если вначале ускорить процесс, то он не кликает по второй кнопке NEXT, оэтому убрала этот метод
       navigationNext();
       navigationNext();
       this.waitForElementPresent(By.id(SIGN_IN_BUTTON), "Cannot find Sign in Button on EnterForm ", 15);
    }

    public void passOnboardingFull(String locale){
        this.defineLocaleOnboarding(locale);
        this.waitForElementPresent(By.xpath(getOnboardingPage1TitleXpath(onboarding_page1_title)), "cannot find title onboarding 1", 15);
        this.waitForElementPresent(By.xpath(getOnboardingPage1TextXpath(onboarding_page1_text)), "cannot find description onboarding 1", 15);
        navigationNext();
        this.waitForElementPresent(By.xpath(getOnboardingPage2TitleXpath(onboarding_page2_title)), "cannot find title onboarding 2");
        this.waitForElementPresent(By.xpath(getOnboardingPage2TextXpath(onboarding_page2_text)), "cannot find description onboarding 2");
        navigationNext();
        this.waitForElementPresent(By.id(SIGN_IN_BUTTON), "Cannot find Sign in Button on EnterForm ", 15);
    }
